        ...
        switch (error.response.data) {
            ...
            case 3:
                error.message = translate(
                    `validationErrors.folder.${error.response.data}`,
                );
                break;
            default:
                error.message =
                    translate('validationErrors.default406') +
...
export const requestFolderUpdate = (folder, permissions) => (
    dispatch,
    getState,
) => {
    const {username, sessionKey} = getState().authentication;
...
export const requestFolderCreation = (folder, permissions) => (
    dispatch,
    getState,
) => {
    const {username, sessionKey} = getState().authentication;
    ...
    return axios({
        ...
        .then(
            response =>
                new Promise(resolve => {
                    if (!folder.parent) dispatch(requestFolderContents()); // Root folder created, cause a repopulation of the drawer folders, fetchRootFolders
                    resolve(response.data); // pass request data along
                }),
        )
        .catch(parse406Error);
            ...
            return new Promise(resolve => {
                if (response.data.search === getState().search.value) {
                    const cleanContents = {
                        ...
                        credentials: response.data.credentials.map(cred => ({
                            ...cred,
                            password: networkDecode(cred.password),
                        })),
                    };
            ...
            try {
                isCurrentUserAdmin =
                    response.data.permissions.find(
                        p => p.userId == getState().authentication.username,
                    ).hasAdmin == '1';
            } catch (err) {
                isCurrentUserAdmin = false;
            }
